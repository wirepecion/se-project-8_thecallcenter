...
export default function FacilityCatalog({ facilities }: { facilities?: FacilityItem[] }) {
    ...
    const filteredFacilities = (facilities: FacilityItem[]) => {
        return sortedFacilities(facilities).filter(facility => {
            ...
            if (onFilter && facility.rank === 'silver' && (userProfile?.membershipTier === 'none' || userProfile?.membershipTier === 'bronze')) {
                return false;
            }
            if (onFilter && facility.rank === 'gold' && (userProfile?.membershipTier === 'none' || userProfile?.membershipTier === 'bronze' || userProfile?.membershipTier === 'silver')) {
                return false;
            }
            if (onFilter && facility.rank === 'platinum' && (userProfile?.membershipTier === 'none' || userProfile?.membershipTier === 'bronze' || userProfile?.membershipTier === 'silver' || userProfile?.membershipTier === 'gold')) {
                return false;
            }
            if (onFilter && facility.rank === 'diamond' && (userProfile?.membershipTier === 'none' || userProfile?.membershipTier === 'bronze' || userProfile?.membershipTier === 'silver' || userProfile?.membershipTier === 'gold' || userProfile?.membershipTier === 'platinum')) {
                return false;
            }
            return true;
        }
